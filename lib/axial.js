...
class AxialSchema extends AxialObject {
  constructor (name, prototype, rootSchema) {
    super();
    ...
    if (this._name) {
      Axial.Schema[name.replace(/\./g, '_')] = this;
    }
  }
  ...
  from (prototype) {
    for (let key in prototype) {
      if (prototype.hasOwnProperty(key)) {
        ...
        const path = this._name ? this._name + '.' + key : key;
...
let Axial = window.Axial = {
  define (name, prototype) {
    const schema =  new AxialSchema(name, prototype);
    const _name = schema.name;
