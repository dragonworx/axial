module.exports = (function Define_Axial () {
  ...
  class AxialInterface extends AxialObject {
    constructor (interfaceId = BLANK_INTERFACE_NAME, prototype, rootInterface) {
      super();
    ...
    create (defaultValues = {}, container) {
      // create instance
  ...
  class AxialInstanceProxy {
    constructor (instance, iface, container) {
      instance[PROXY_KEY] = this;
      instance['_type'] = iface.id;
      instance['_typeOf'] = this.typeOf.bind(this);
      this._instanceOf = iface.id;
    ...
    dock (container, property) {
      if (typeof container.onChildDocking === 'function') {
        const dockError = container.onChildDocking(this._instance, property);
      ...
      const event = {
        ...
        preserveParent: () => {
          this._container = oldContainer;
        }
      };

      if (typeof this._instance.onDock === 'function') {
        event.dockValue = this._instance.onDock(event);
      }
    ...
    keys () {
      return [...this._iface._properties.keys()];
    }
      ...
      this._watchIntervalId = setInterval(() => {
        ...
        for (let key in instance) {
          if (instance.hasOwnProperty(key) && !_isValidKey(key) && !props.has(key)) {
            ...
            delete instance[key];
            throw new AxialUnexpectedProperty(key, this._iface, this);
          }
        }
      }, 30);
      return true;
    ...
    toPlainObject () {
      try {
        ...
        for (let key of this._iface._properties.keys()) {
          const property = this._iface._properties.get(key);
          if (!property.exports) {
            debugger;
  ...
  util = {
    ...
    stringify (value, refs = []) {
      ...
      if (value instanceof AxialInstance) {
        ...
        const props = _proxy(value).map((k, v) => k + ':' + this.stringify(v, refs));
