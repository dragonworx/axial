module.exports = (function Define_Axial () {
  ...
  let _legalInstanceProperties = ['_state', '_iface', '_listeners', '_parentInstance' ,'_isWatching', '_watchIntervalId'];
  const _arrayMembers = ['concat', 'copyWithin', 'entries', 'every', 'fill', 'filter', 'find', 'findIndex', 'forEach', 'includes', 'indexOf', 'join', 'keys', 'lastIndexOf', 'map', 'pop', 'push', 'reduce', 'reduceRight', 'reverse', 'shift', 'slice', 'some', 'sort', 'splice', 'toLocaleString', 'toSource', 'toString', 'unshift', 'values'];
  const _arrayMutators = ['copyWithin', 'fill', 'pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift'];
    ...
    set _watch (bool) {
      ...
      if (bool) {
        ...
        this._watchIntervalId = setInterval(() => {
          for (let key in this) {
            if (this.hasOwnProperty(key) && _legalInstanceProperties.indexOf(key) === -1 && !props.has(key)) {
              clearInterval(this._watchIntervalId);
